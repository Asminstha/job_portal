...
@section('content')
    <div class="min-h-[80vh] flex items-center justify-center py-12 px-4">
        <div class="w-full max-w-lg">
            ...
            <div class="card p-8">
                <form method="POST" action="{{ route('register.company.post') }}" x-data="{ submitting: false }"
                            ...
                            <div>
                                ...
                                <select id="industry" name="industry"
                                    ...
                                    @foreach (['Information Technology', 'Software Development', 'BPO / KPO', 'Banking & Finance', 'Healthcare', 'Education & Training', 'Media & Communications', 'E-Commerce', 'Manufacturing', 'Hospitality', 'Others'] as $ind)
                                        <option value="{{ $ind }}"
                                            {{ old('industry') === $ind ? 'selected' : '' }}>{{ $ind }}</option>
                                    @endforeach
                                </select>
                                @error('industry')
                                    <p class="form-error">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="name" class="form-label">Your full name <span
                                        class="text-red-500">*</span></label>
                   ...
                   disabled:opacity-60 disabled:cursor-not-allowed">
                        <span x-show="!submitting">Start Free Trial</span>
            ...
            <p class="text-center text-sm text-gray-500 dark:text-gray-400 mt-5">
                ...
                <a href="{{ route('login') }}" class="text-brand-600 dark:text-brand-400 font-medium hover:underline">Sign
                    in</a>
            </p>
        </div>
    </div>
