    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-900
        ...
        <div class="flex items-center justify-between h-16 px-5
            ...
            <button @click="sidebarOpen = false"
                    class="lg:hidden p-1 rounded text-gray-400 hover:text-gray-600">
    ...
    <div class="flex items-center gap-3">
        @if(auth()->user()->avatar)
            <img src="{{ auth()->user()->avatarUrl() }}"
                 ...
                 class="w-10 h-10 rounded-full object-cover
                        border-2 border-purple-200 dark:border-purple-800 flex-shrink-0">
        @else
            <div class="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-900
                        flex items-center justify-center
                        text-purple-700 dark:text-purple-300 font-bold text-sm flex-shrink-0">
        ...
        <div class="min-w-0">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 truncate">
                {{ auth()->user()->seekerProfile?->headline ?? 'Job Seeker' }}
            </p>
        </div>
    </div>
</div>
        ...
        <nav class="px-3 py-4 space-y-1">
            ...
            <p class="px-3 text-xs font-semibold text-gray-400 dark:text-gray-500
                      uppercase tracking-wider mb-2">My Account</p>

            <a href="{{ route('seeker.dashboard') }}"
               class="sidebar-link {{ request()->routeIs('seeker.dashboard') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('seeker.profile.edit') }}"
               class="sidebar-link {{ request()->routeIs('seeker.profile.*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('seeker.applications.index') }}"
               class="sidebar-link {{ request()->routeIs('seeker.applications.*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('seeker.saved.index') }}"
               class="sidebar-link {{ request()->routeIs('seeker.saved.*') ? 'active' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <header class="h-16 bg-white dark:bg-gray-900
            ...
            <div class="flex items-center gap-3">
                <button @click="darkMode = !darkMode"
                    ...
                    <svg x-show="!darkMode" class="w-5 h-5" fill="none"
        ...
        @if(session('success'))
            <div x-data="{ show: true }" x-show="show"
                 ...
                 class="mx-4 sm:mx-6 mt-4 p-3 bg-green-50 dark:bg-green-900/30
                        ...
                        text-green-800 dark:text-green-200 rounded-lg text-sm
